...
def generateYaml(member, memberPath, flag,workerNodeDir):
	if flag == "/peers":
		configPEERS(member, memberPath,workerNodeDir)
	else:
		configORDERERS(member, memberPath,workerNodeDir) 
	

# create peer/pod and cli pod
def configPEERS(name, path,workerNodeDir): # name means peerid.
...
# create orderer/pod
def configORDERERS(name, path,workerNodeDir): # name means ordererid
